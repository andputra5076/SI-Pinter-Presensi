...
class _DataSiswaState extends State<DataSiswa> {
  ...
  late Future<List<Map<String, dynamic>>> futureDataSiswa;
  Future<List<Map<String, dynamic>>> fetchDataSiswa() async {
    final response =
        await http.get(Uri.parse('192.168.1.9/pesantrenadmin/api/get-siswa'));
  ...
  String _searchQuery = '';
  String _activeMenuItem = 'DATA SISWA';
  ...
  Future<void> _fetchKelasList() async {
    final response = await http.get(Uri.parse('192.168.1.9/pesantrenadmin/api/get-kelas'));

    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      setState(() {
        _kelasList = data.map((item) => item['nama'] as String).toList();
        _kelasList.insert(0, 'Semua Kelas'); // Add a default option
      });
    } else {
      // Handle error response
      print('Failed to load kelas list');
    }
  }
                                ...
                                return LayoutBuilder(
                                  builder: (context, constraints) {
                                    ...
                                    final double columnWidth = isMobile
                                        ...
                                        : constraints.maxWidth /
                                            7; // Distribute available width for larger screens
                                      ...
                                      child: ConstrainedBox(
                                        ...
                                        child: DataTable(
                                          ...
                                          columns: [
                                            ...
                                            DataColumn(
                                              label: Container(
                                                width: columnWidth, // Wider for 'Tahun Ajaran'
                                                child: Align(
                                                  alignment:
                                                      Alignment.centerLeft,
                                                  child: Text(
  ...
  void _showEditDialog(Map<String, dynamic> item) {
    ...
    final nisController = TextEditingController(text: item['nis']);
    final nisnController = TextEditingController(text: item['nisn']);
    final id = item['id'].toString();
    String? selectedKelas = item['id_kelas'];
    String? selectedTahunAjaran = item['id_tahun_ajaran'];
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final response = await http.post(
                      Uri.parse('192.168.1.9/pesantrenadmin/api/edit-siswa'),
                      headers: {
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final response = await http.post(
                  Uri.parse('192.168.1.9/pesantrenadmin/api/hapus-siswa'),
                  headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                  },
                  body: {
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                if (disable == false) {
                  ...
                  // Send data to API
                  final uri =
                      Uri.parse('192.168.1.9/pesantrenadmin/api/import-siswa');
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final response = await http.post(
                      Uri.parse(
                          '192.168.1.9/pesantrenadmin/api/tambah-siswa'),
                      headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                      },
                      body: data,
